#!/usr/bin/env python3
"""
Update strawberry farms with accurate GPS coordinates using Google Places API.
...
def update_strawberry_coordinates(api_key, farms_data):
    """Update strawberry farms with accurate GPS coordinates."""
    updated_count = 0
    skipped_count = 0
    
    print(f"Processing {len(farms_data)} strawberry farms...")
...
def main():
    """Main function."""
    print("Strawberry Farms GPS Coordinate Update")
    print("=" * 40)
    ...
    # Load current data
    print(f"Loading data from {strawberries_file.name}...")
    farms_data = load_json_file(strawberries_file)
    
    if not farms_data:
        print("Failed to load strawberry farms data")
        return
    
    # Update coordinates
    updated_data = update_strawberry_coordinates(api_key, farms_data)
    
    # Save updated data
    print(f"\nSaving updated data to {strawberries_file.name}...")
    if save_json_file(strawberries_file, updated_data):
        print("✅ Successfully saved updated strawberry farms data with accurate coordinates")
    else:
