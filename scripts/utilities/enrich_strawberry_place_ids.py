#!/usr/bin/env python3
"""
Enrich strawberry farms with proper Google Place IDs using Google Maps API.
...
def enrich_strawberry_farms(api_key, farms_data):
    """Enrich strawberry farms with proper place IDs."""
    enriched_count = 0
    skipped_count = 0
    ...
    for i, farm in enumerate(farms_data, 1):
        ...
        if result:
            ...
            farm['place_query'] = f"{name}, strawberries, NY"
...
def main():
    """Main function."""
    print("Strawberry Farms Place ID Enrichment")
    print("=" * 40)
    ...
    # Enrich the data
    enriched_data = enrich_strawberry_farms(api_key, farms_data)
    
    # Save updated data
    print(f"\nSaving updated data to {strawberries_file.name}...")
    if save_json_file(strawberries_file, enriched_data):
        print("✅ Successfully saved updated strawberry farms data")
    else:
